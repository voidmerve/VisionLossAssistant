...
# 🔹 Kamerana göre kalibre et — test sonrası ayarla (genelde 0.7 - 1.3 arası)
...
def estimate_depth(frame_rgb):
    ...
    YER PİKSELLERİ ELENİR, sadece nesne yüksekliği baz alınır."""
    try:
        t0 = time.time()
        ...
        # 🔹 Sadece görüntünün ORTA YÜKSEKLİĞİNİ al
        # Üst %30 → tavan/gökyüzü (atla)
        # Alt %35 → yer/zemin (atla)
        # Orta %35 → nesnelerin gövde yüksekliği (KULLAN)
        top_cut = int(h * 0.30)
        bottom_cut = int(h * 0.65)
        roi = depth_map[top_cut:bottom_cut, :]

        def median_of_closest(region, percent=0.30):
            """En yakın %30'un MEDYANI (outlier'ları eler)."""
            flat = region.flatten()
            flat = flat[flat > 0.1]   # 0'a yakın hatalı pikselleri at
            flat = flat[flat < 15.0]  # 15m üstü önemsiz

            if len(flat) == 0:
                return 5.0  # default uzak
